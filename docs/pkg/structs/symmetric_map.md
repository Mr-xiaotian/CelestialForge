...
本文件定义了 `SymmetricMap` 泛型类型，实现了一个双向一对一映射（对称映射）。每个元素最多出现一次，通过 `forward` 和 `reverse` 两个内部字典实现双向查找。`Set` 操作是幂等的，当设置新的配对关系时会自动解除旧的冲突配对。可以通过 `allowSelf` 参数控制是否允许元素与自身配对。该类型在 `tests/structs_test.go` 中有完整的测试覆盖。
